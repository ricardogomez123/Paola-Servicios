...
namespace CertificadorWs.Business
{
    public class ValidadorDatos
    {
        ...
        // CertificadorWs.Business.ValidadorDatos
        public int ValidarSelloRetencion(string cadena, byte[] firma, X509Certificate2 certificado, ref string sha1Hash)
        {
            int result;
            try
            {
            ...
            {
                ValidadorDatos.Logger.Error("", ee);
                result = 302;
            }
            return result;
        }
        ...
        public int ValidarSello(string cadena, byte[] firma, X509Certificate2 certificado, ref string sha1Hash)
        {
            ...
            {/*
               ...
               */
                ...
                sha1Hash = BitConverter.ToString(hash);
                sha1Hash = sha1Hash.Replace("-", "");
                var res = rsa.VerifyHash(hash, CryptoConfig.MapNameToOID("SHA256"), firma) ? 0 : 302;
                if (res == 0)
                    return res;
                var valido = rsa.VerifyData(Encoding.UTF8.GetBytes(cadena), new SHA256Managed(), firma);
                return valido ? 0 : 302;
